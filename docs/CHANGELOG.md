...
- **HAMALI viewer role** — migration `039`: `users.hamali_worker_id` links HAMALI logins to `hamali_workers` roster; `HAMALI` role (`hamali:read`, `dashboard:read`); APIs `GET /hamali/me/daily`, `GET /hamali/me/summary` (scoped to linked worker); auto-create `hamali_workers` row when admin creates HAMALI user; mobile catalog `HAMALI_VIEW`; web nav shows read-only **Operations → Hamali** for HAMALI users.
