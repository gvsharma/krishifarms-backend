...
- **HAMALI viewer role** — migration `035` `hamali_work_entries` + `HAMALI` role (read-only bags/tips); APIs `GET /hamali/me/daily`, `/hamali/me/summary`, CRUD `/hamali/work-entries`, `/hamali/workers`; auto-create `workers` row when admin creates HAMALI user; mobile catalog `HAMALI_VIEW`; web `/hamali` portal + Settings → Hamali work log for supervisors.
