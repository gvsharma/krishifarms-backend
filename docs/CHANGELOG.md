...
- **`scripts/purge_all_org_data.py`** — FK-safe wipe of all org business data for a fresh UAT start; keeps org, roles, permissions, and OWNER login (`--confirm PURGE-ALL-DATA`; optional `--keep-master-data`). Runbook in [DEMO_DATA.md](./DEMO_DATA.md#fresh-start-purge-all-data).
- **Procurement buyer assignment + sale margin (Part B)** — migration `042` adds `procurements.sale_rate_per_quintal` + `sale_date`. New `POST /procurements/assign-buyer` bulk-assigns one buyer (+ optional sale rate, dispatch date, payment terms) to many procurements (identified by `{procurement_id, procurement_date}` since the table is date-partitioned). `PATCH /procurements/{id}` also accepts `sale_rate_per_quintal` / `sale_date`. Staff `profit_summary` now includes `sale_rate_per_quintal`, `sale_amount`, and `sale_margin_amount` (= `(sale_rate − farmer_rate) × net_quintals`) folded into `total_profit_amount`. Web: new **Sales → assign buyer** screen (`/procurement/sales`, multi-select + dialog) and buyer/sale-rate/margin shown on the ticket detail.
