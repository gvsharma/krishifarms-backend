...
- **Tractor implement default rates** — `vehicle_types.default_rate` + `default_rate_unit` (`hour`/`trip`/`bale`); migration `041` seeds Cultivator ₹1200/hr, Rotavator ₹1440/hr, Trolley ₹250/trip, Baler ₹40/bale, Weeder ₹1000/hr, Fertilizer pump ₹500/hr. Web tractor-work form auto-calculates total from equipment + hours/trips/bales; backend applies when total is zero.
- **Farmer 360 activity visibility** — overview **Recent activity** feed; services/procurements tables link to detail pages; timeline links; service rows show activity type, trips/bales; tractor hours analytics count `tractor_work` only.
...
- **Field service new form (tractor work)** — `/field-services/new?farmer_id=&vehicle=tractor` pre-fills farmer, village, equipment, and default hourly rate; duration as separate hours + minutes; total and pending auto-calculated (read-only) from rate × time minus advance. — web `/procurement/new` no longer collects a buyer; it saves in one step via `POST /procurements/field-entry` (`auto_confirm`), so the ticket is recorded as **confirmed** with weights/rate/amounts populated immediately (no `draft → submit → weigh → price → confirm`). Buyer is assigned later (Part B, upcoming). Finalized tickets are now editable: `PATCH /procurements/{id}` accepts `weight_per_bag_kg`, `moisture_pct`, `rate_per_quintal` and recomputes weights/amounts; when a **confirmed** ticket's payable changes it posts reversing + corrected farmer-ledger entries (immutable ledger), and edits are blocked once a farmer payment is allocated. Web: new `/procurement/[id]/edit` page + **Edit** action on the detail page.
